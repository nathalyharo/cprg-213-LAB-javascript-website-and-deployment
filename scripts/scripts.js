...
function recalculate() {
    ...
    let totalCost = 0; // initiates the total cost 
    if (modelName == "Model XYZ") {
        totalCost = duration * 100;
    } else {
        totalCost = duration * 213;
    }
    costLabel.innerHTML = totalCost; // will return the total cost value into the span id='calculated-cost element
}
...
function changeDuration() {
    ...
    duration = parseInt(prompt("Enter new duration: ")); // convert to int so it can be used to calculate total cost
    recalculate();
    durationText.innerHTML = duration; // displays duration user enters 
}
